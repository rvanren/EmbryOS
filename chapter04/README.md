...
- Transition from cooperative to preemptive multitasking, where the timer automatically triggers.
context switches.
- Understand how periodic interrupts are generated by the RISC–V machine timer.
...
| `trap_wfi(block)` | Wait for the next interrupt and invoke `software_trap_handler()` with a pointer to the trap frame |
...
call `user_yield()`, which moves the process to priority 1.
The idle loop, invoked by the first process, runs at priority 2.
The idle loop using the RISC-V `wfi` instruction to wait for an interrupt,
...
You see here a time interrupt followed by a context switch, which is process pre-emption in action!
