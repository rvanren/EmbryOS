...
void software_trap_handler(struct trap_frame *tf) {
    if (tf->scause & SCAUSE_INTERRUPT_MASK) {
        // Interrupts
        ...
        // Exceptions (includes system calls)
        switch (tf->scause & SCAUSE_CODE_MASK) {
...
int intr_init(void) {
    ...
    uword_t handler = (uword_t)_trap_handler;

    // Set S-mode trap vector
    asm volatile ("csrw stvec, %0" :: "r"(handler));

    // Enable supervisor external (SEIE bit 9) and timer (STIE bit 5) interrupts
    asm volatile ("csrs sie, %0" :: "r"((uword_t)1 << 9));
    asm volatile ("csrs sie, %0" :: "r"((uword_t)1 << 5));

    // Set SIE bit in sstatus (bit 1)
