...
static const struct simple_inode_block null_block = { 0 };
...
int simple_alloc(void *st) {
    ...
    if (inode == 0) return 0;
    s->lower->write(s->lower->state, s->inode_below, inode, &null_block);
    return inode;
}
...
void simple_read(void *st, int inode, int blk, void *dst) {
    ...
    struct simple_state *s = st;
    static struct simple_inode_block ib;
    s->lower->read(s->lower->state, s->inode_below, inode, &ib);
    int b = blk < SIMPLE_N_POINTERS ? ib.blocks[blk] : 0;
...
void simple_write(void *st, int inode, int blk, const void *src) {
    ...
    struct simple_state *s = st;
    if ((unsigned) blk >= SIMPLE_N_POINTERS) die("simple_write: bad offset");
    static struct simple_inode_block ib;
    s->lower->read(s->lower->state, s->inode_below, inode, &ib);
    if (ib.blocks[blk] == 0) {
